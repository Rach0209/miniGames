...
const GAME_TYPE = 'flappy-bird';
const LOCAL_BEST_KEY = 'flappy_best_score_v1';
const LOCAL_GAMES_KEY = 'flappy_total_games_v1';
...
export async function loadFlappyBirdBest(): Promise<number> {
...
export async function recordFlappyBirdGame(score: number): Promise<void> {
  const user = await getCurrentUser();
