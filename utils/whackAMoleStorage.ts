...
const GAME_TYPE = 'whack-a-mole';
const LOCAL_BEST_KEY = 'whack_best_score_v1';
const LOCAL_GAMES_KEY = 'whack_total_games_v1';
...
export async function loadWhackBest(): Promise<number> {
...
export async function recordWhackGame(score: number): Promise<void> {
  const user = await getCurrentUser();
